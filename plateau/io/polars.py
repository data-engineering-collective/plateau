...
def read_dataset_as_dataframes(
    ...
    """Read a dataset as a list of polars dataframes.
    ...
    List[polars.DataFrame]
        Returns a list of polars.DataFrame. One element per partition
    ...
    .. code ::
        ...
        >>> import minimalkv
        >>> from plateau.io.polars import read_dataset_as_dataframes
...
def read_dataset_as_metapartitions(
    ...
    .. code ::
        ...
        >>> import minimalkv
        >>> from plateau.io.polars import read_dataset_as_dataframe
...
def read_table(
    ...
    single polars dataframe in one go. Mostly useful for smaller tables or datasets
    ...
    -------
    polars.DataFrame
        Returns a polars.DataFrame holding the data of the requested columns
    ...
    .. code ::
        ...
        >>> import minimalkv
        >>> from plateau.io.polars import read_table
...
def commit_dataset(
    ...
    1. Add previously written partitions to this dataset
        ...
        :func:`~plateau.io.polars.write_single_partition` and :func:`~plateau.io.polars.create_empty_dataset_header`.

        .. code::

            import polars as pl
            from plateau.io.polars import write_single_partition, commit_dataset
...
def store_dataframes_as_dataset(
    store: KeyValueStore,
    dataset_uuid: str,
    dfs: list[pl.DataFrame | dict[str, pl.DataFrame]],
    ...
    """Utility function to store a list of polars dataframes as a partitioned dataset
    ...
    if isinstance(dfs, pl.DataFrame):
        raise TypeError(
            f"Please pass a list of polars.DataFrame as input. Instead got {type(dfs)}"
        )
...
def create_empty_dataset_header(
    ...
    combination with :func:`~plateau.io.polars.write_single_partition` to create
...
def write_single_partition(
    ...
    dataset_uuid: str | None = None,
    data: pl.DataFrame | dict[str, pl.DataFrame] | None = None,
    df_serializer: DataFrameSerializer | None = None,
    ...
    """Write a single partition of a dataset.

    Parameters
    ----------
    store: Optional[KeyValueStore]
        The store where the partition will be written
    dataset_uuid: Optional[str]
        The unique identifier for the dataset
    data: Optional[Union[polars.DataFrame, Dict[str, polars.DataFrame]]]
        The data to write. Can be either a single DataFrame or a dictionary mapping
        table names to DataFrames
    df_serializer: Optional[DataFrameSerializer]
        Serializer to use for the dataframes
    ...
    factory: Optional[DatasetFactory]
        The dataset factory to use
    secondary_indices: Optional[List[str]]
        Columns to create secondary indices for
    table_name: str
        Name of the table

    Returns
    -------
    plateau.core.dataset.DatasetMetadata
        The metadata of the updated dataset
    """
    ds_factory = _ensure_factory(
        dataset_uuid=dataset_uuid,
        store=store,
    ...
    # Normalize input to MetaPartition
    ...
    return update_dataset_from_partitions(
        [mp],
        store_factory=ds_factory.store,
        dataset_uuid=dataset_uuid,
        ds_factory=ds_factory,
    )
...
def update_dataset_from_dataframes(
    df_list: list[pl.DataFrame | dict[str, pl.DataFrame]],
    store: KeyValueStore | None = None,
    dataset_uuid: str | None = None,
    delete_scope=None,
    metadata=None,
    ...
    Parameters
    ----------
    df_list: List[Union[polars.DataFrame, Dict[str, polars.DataFrame]]]
        The list of dataframes to update the dataset with
    store: Optional[KeyValueStore]
        The store where the dataset is persisted
    dataset_uuid: Optional[str]
        The unique identifier for the dataset
    delete_scope: Optional[List[Dict[str, Any]]]
        The scope of data to delete
    metadata: Optional[Dict[str, Dict[str, Any]]]
        Metadata to update with the dataset
    df_serializer: Optional[DataFrameSerializer]
        Serializer to use for the dataframes
    ...
    # Normalize input to list of MetaPartitions
    mps = []
    for df in df_list:
        mp = parse_input_to_metapartition(
            df,
        ...
        if sort_partitions_by:
            mp = mp.apply(lambda df: df.sort(sort_partitions_by))
...
def build_dataset_indices(store, dataset_uuid, columns, factory=None):
    ...
    the indices to the dataset. The dataset partitions themselves are not mutated.
