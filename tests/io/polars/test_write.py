...
def test_write_single_partition(store_factory, mock_uuid, metadata_version):
    create_empty_dataset_header(
        store=store_factory(),
        schema=pl.DataFrame({"col": [1]}),
        dataset_uuid="some_dataset",
        metadata_version=metadata_version,
        table_name="table1",
    )

    new_data = pl.DataFrame({"col": [1, 2]})
    ...
    expected_mp = MetaPartition(
        ...
        metadata_version=4,
        schema=make_meta(pl.DataFrame({"col": [1, 2]}), origin="table1"),
    )
    ...
    with pytest.raises(ValueError):
        ...
        new_data = pl.DataFrame({"col": [datetime.date(2010, 1, 1)]})
        write_single_partition(
            store=store_factory,
            dataset_uuid="some_dataset",
            data=new_data,
...
def test_create_dataset_header(store, metadata_storage_format, frozen_time):
    schema = make_meta(pl.DataFrame({"col": [1]}), origin="1")
